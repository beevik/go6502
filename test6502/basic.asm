...
Z1d     .EQ       $1D
ch      .EQ       $24     ; horizontal cursor location
var     .EQ       $48
lomem   .EQ       $4A     ; lower limit of memory used by BASIC (2 bytes)
himem   .EQ       $4C     ; upper limit of memory used by BASIC (2 bytes)
rnd     .EQ       $4E     ; random number (2 bytes)
...
noun_stk_l .EQ    $50
syn_stk_h .EQ     $58     ; through $77
noun_stk_h_str .EQ $78
syn_stk_l  .EQ    $80     ; through $9F
noun_stk_h_int .EQ $A0
txtndxstk  .EQ    $A8     ; through $C7
text_index .EQ    $C8     ; index into text being tokenized (in buffer at $0200)
leadbl  .EQ       $C9     ; leading blanks
pp      .EQ       $CA     ; pointer to end of program (2 bytes)
pv      .EQ       $CC     ; pointer to end of variable storage (2 bytes)
acc     .EQ       $CE     ; (2 bytes)
srch    .EQ       $D0
tokndxstk .EQ     $D1
srch2   .EQ       $D2
...
run_flag .EQ      $D9
aux     .EQ       $DA
pline   .EQ       $DC     ; pointer to current program line (2 bytes)
pverb   .EQ       $E0     ; pointer to current verb (2 bytes)
p1      .EQ       $E2
p2      .EQ       $E4
p3      .EQ       $E6
token_index .EQ   $F1    ; pointer used to write tokens into buffer  2 bytes)
pcon    .EQ       $F2    ; temp used in decimal output (2 bytes)
auto_inc .EQ      $F4
auto_ln .EQ       $F6
auto_flag .EQ     $F8
char    .EQ       $F9
leadzr  .EQ       $FA
for_nest_count .EQ $FB    ; count of active (nested) FOR loops
gosub_nest_count .EQ $FC  ; count of active (nested) subroutines calls (GOSUB)
synstkdx .EQ      $FD
synpag  .EQ       $FE

; GOSUB stack, max eight entries
; note that the Apple II version has sixteen entries
gstk_pverbl     .EQ       $0100    ; saved pverb
gstk_pverbh     .EQ       $0108
gstk_plinel     .EQ       $0110    ; saved pline
gstk_plineh     .EQ       $0118

; FOR stack, max eight entries
; note that the Apple II version has sixteen entries
fstk_varl       .EQ       $0120   ; pointer to index variable
fstk_varh       .EQ       $0128
fstk_stepl      .EQ       $0130   ; step value
fstk_steph      .EQ       $0138
fstk_plinel     .EQ       $0140   ; saved pline
fstk_plineh     .EQ       $0148
fstk_pverbl     .EQ       $0150   ; saved pverb
fstk_pverbh     .EQ       $0158
fstk_tol        .EQ       $0160   ; "to" (limit) value
fstk_toh        .EQ       $0168
buffer  .EQ       $0200
KBD     .EQ       $D010
KBDCR   .EQ       $D011
...
Le2b6  LSR     run_flag
        LDA     #$BE        ; Prompt character (high bit set)
        JSR     cout
        LDY     #$00
        STY     leadzr
        BIT     auto_flag
...
prdec  STA     pcon+1  ; output AX in decimal
        STX     pcon
        LDX     #$04
        STX     leadbl
Le523  LDA     #$B0
...
return_stmt    LDY     #$4A
        ...
        LDA     gstk_plineh-1,Y
        STA     pline+1
        LDX     synpag+1,Y            ; force absolute addressing mode
        LDA     gstk_pverbh-1,Y
Le8be  TAY
        TXA
        JMP     Le87a
